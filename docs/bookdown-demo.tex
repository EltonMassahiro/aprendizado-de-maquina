...
Dado uma matriz \(A_{nxn}\) que define uma transformação linear (não muda sua dimensão), existem vetores onde sua orientação não é afetada por esta transformação, os \textbf{autovetores}.
...
A qualidade de aproximação do modelo depende do número de componentes mantidas no sistema e pode ser medida com a \textbf{avaliação da proporção da variância total explicada}.
